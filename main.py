...
def redraw_window(screen, board, time):
    ...
    text = fnt.render("Time: " + timeToString(time), 1, BLACK)
...
class GameController(object):
    ...
    def checkEvent(self):
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if self.board.cubes[i][j].temp != 0:
                        if self.board.place(self.board.cubes[i][j].temp):
                            self.board.cubes[i][j].flag =True
                            print("Success")
                        else:
                            self.board.cubes[i][j].flag = False
                            print("Wrong")
                        self.val = None
        ...
        redraw_window(self.screen, self.board, play_time)
